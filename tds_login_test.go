...
type testFedAuthProvider struct {
...
func (p *testFedAuthProvider) ConfigureProvider(fa *FederatedAuthenticationState) error {
	if p.configureError != nil {
		return p.configureError
	}

	fa.FedAuthLibrary = p.library
	fa.ADALWorkflow = p.workflow

	return nil
}

func (p *testFedAuthProvider) ProvideSecurityToken(ctx context.Context) (string, error) {
	if p.tokenError != nil {
		return "", p.tokenError
	}
...
func (p *testFedAuthProvider) ProvideActiveDirectoryToken(ctx context.Context, serverSPN, stsURL string) (string, error) {
	if p.tokenError != nil {
		return "", p.tokenError
	}
...
func TestLoginWithSecurityTokenAuth(t *testing.T) {
	conn, err := NewConnector("sqlserver://localhost:1433?Workstation ID=localhost&log=128")
	if err != nil {
		t.Errorf("Unable to parse dummy DSN: %v", err)
	}

	conn.FederatedAuthenticationProvider = &testFedAuthProvider{
		library: fedAuthLibrarySecurityToken,
	}
...
func TestLoginWithADALUsernamePasswordAuth(t *testing.T) {
	conn, err := NewConnector("sqlserver://localhost:1433?Workstation ID=localhost&log=128&fedauth=ActiveDirectoryPassword")
	if err != nil {
		t.Errorf("Unable to parse dummy DSN: %v", err)
	}

	conn.FederatedAuthenticationProvider = &testFedAuthProvider{
		library:  fedAuthLibraryADAL,
		workflow: fedAuthADALWorkflowPassword,
	}
...
func TestLoginWithADALManagedIdentityAuth(t *testing.T) {
	conn, err := NewConnector("sqlserver://localhost:1433?Workstation ID=localhost&log=128&fedauth=ActiveDirectoryMSI")
	if err != nil {
		t.Errorf("Unable to parse dummy DSN: %v", err)
	}
	...
	conn.FederatedAuthenticationProvider = &testFedAuthProvider{
		library:  fedAuthLibraryADAL,
		workflow: fedAuthADALWorkflowMSI,
	}
