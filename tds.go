...
initiate_connection:
	...
	switch {
	case login.FedAuthLibrary == fedAuthLibrarySecurityToken:
		...
		login.FedAuthToken, err = p.securityTokenProvider(ctx)
		if err != nil {
			if p.logFlags&logDebug != 0 {
	...
	case login.FedAuthLibrary == fedAuthLibraryADAL && login.FedAuthADALWorkflow == fedAuthADALWorkflowPassword:
		if p.logFlags&logDebug != 0 {
			log.Printf("Starting ADAL username/password workflow for user %s", p.user)
		}
		login.UserName = p.user
		login.Password = p.password
	case login.FedAuthLibrary == fedAuthLibraryADAL && login.FedAuthADALWorkflow == fedAuthADALWorkflowMSI:
		if p.logFlags&logDebug != 0 {
			log.Println("Starting ADAL managed service identity (MSI) workflow")
		}
	case login.FedAuthLibrary == fedAuthLibraryADAL:
		...
		for tok := range tokchan {
			...
			case fedAuthInfoStruct:
				...
				if login.FedAuthLibrary == fedAuthLibraryADAL {
					...
					login.FedAuthToken, err = p.activeDirectoryTokenProvider(ctx, token.ServerSPN, token.STSURL)
					if err != nil {
						return nil, err
					}
