...
// Implement featureExt interface based on federated authentication state
func (e *FederatedAuthenticationState) featureID() byte {
	return featExtFEDAUTH
}

func (e *FederatedAuthenticationState) toBytes() []byte {
	if e == nil {
		return nil
	}
...
func sendFedAuthInfo(w *tdsBuffer, fedAuth *FederatedAuthenticationState) (err error) {
...
func configureFedAuth(c *Connector, p connectParams) (fe *FederatedAuthenticationState, err error) {
	if c == nil || c.FederatedAuthenticationProvider == nil {
		return &FederatedAuthenticationState{
			FedAuthLibrary: fedAuthLibraryReserved,
		}, nil
	}

	fe = &FederatedAuthenticationState{
		FedAuthWorkflow: p.fedAuthWorkflow,
		UserName:        p.user,
		Password:        p.password,
		ClientCertPath:  p.aadClientCertPath,

		FedAuthLibrary: fedAuthLibraryReserved,
	}

	err = c.FederatedAuthenticationProvider.ConfigureProvider(fe)
	if err != nil {
		return nil, err
	}
...
func preparePreloginFields(p connectParams, fe *FederatedAuthenticationState) map[uint8][]byte {
...
func interpretPreloginResponse(p connectParams, fe *FederatedAuthenticationState, fields map[uint8][]byte) (encrypt byte, err error) {
...
func prepareLogin(ctx context.Context, c *Connector, p connectParams, log optionalLogger, auth auth, fe *FederatedAuthenticationState, packetSize uint32) (l *login, err error) {
	l = &login{
		TDSVersion:   verTDS74,
	...
	case fe.FedAuthLibrary == fedAuthLibrarySecurityToken:
		...
		fe.FedAuthToken, err = c.FederatedAuthenticationProvider.ProvideSecurityToken(ctx)
		if err != nil {
			if p.logFlags&logDebug != 0 {
	...
	case fe.FedAuthLibrary == fedAuthLibraryADAL:
		...
		l.UserName = fe.UserName
		l.Password = fe.Password
...
initiate_connection:
	...
	fedAuth, err := configureFedAuth(c, p)
	if err != nil {
		return nil, err
	}
	...
	for loginAck := false; !loginAck; {
		...
		for {
			...
			case fedAuthInfoStruct:
				...
				fedAuth.FedAuthToken, err = c.FederatedAuthenticationProvider.ProvideActiveDirectoryToken(ctx, token.ServerSPN, token.STSURL)
				if err != nil {
					return nil, err
				}
